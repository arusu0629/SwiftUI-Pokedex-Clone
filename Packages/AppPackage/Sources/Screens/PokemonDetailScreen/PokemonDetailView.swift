...
// MARK: - PokemonDetailView.backgroundShape
extension PokemonDetailView {
    ...
    private func backgroundShape(_ size: CGSize, shapeColor: Color) -> some View {
        Path { path in
            ...
            // 楕円形を作成し、はねん部分が長方形下部方向にはみだす位置に配置
        ...
        .fill(shapeColor)
        .offset(y: -size.height * (state.isBgAnimationStarted ? 0.75 : 2.0))
        .animation(.spring(), value: state.isBgAnimationStarted)
        .task {
            try? await Task.sleep(for: .seconds(0.1))
            state.updateIsBgAnimationStarted(true)
        }
    }
}

// MARK: - PokemonDetailView.mainVisual
extension PokemonDetailView {
...
// MARK: - PokemonDetailView.description
extension PokemonDetailView {
...
// MARK: - PokemonDetailView.status
extension PokemonDetailView {

    private func status(data: PokemonDetail) -> some View {
        Grid(horizontalSpacing: SpaceToken.s,
             verticalSpacing: SpaceToken.s) {
...
// MARK: - PokemonDetailView.information
extension PokemonDetailView {

    @ViewBuilder
    func information(data: PokemonDetail) -> some View {
...
#Preview {
    PokemonDetailView(
        router: .init(isPresented:
                .constant(.pokemonDetail(number: 1))),
        input: .init(
            withNavigation: false,
            naviBarLeadingButtonType: .back),
        pokemonNumber: 1
    )
}
